...
const historyMiddleware = require('@easy-team/koa-history-api-fallback');
...
class WebpackTool {
  ...
  getPort(target = 'web', offset = 0) {
    const key = `port_${target}_${offset}`;
    if (this.ports[key]) {
      return this.ports[key];
    }
    const configPort =  this.config.port || Number(process.env.EASY_ENV_DEV_PORT) || 9000;
    const targetPort = target === 'web' ? configPort: configPort + offset;
    const port = require('node-tool-utils').getPort(targetPort);
    this.ports[key] = port;
    return port;
  }
